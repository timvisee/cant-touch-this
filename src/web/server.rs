...
fn set_record(record: bool, gesture_controller: State<Arc<GestureController>>) -> Json<RecordResponse> {
    // Set the recording state in the gesture controller
    gesture_controller.set_recording(record);

    // Respond with the state
    Json(RecordResponse { started: record })
}
